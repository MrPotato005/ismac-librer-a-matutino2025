...
public void recomputarTotales(BigDecimal tasaIva) {
    subtotal = items.stream().map(it -> {
        ...
        return (total != null ? total : pu.multiply(BigDecimal.valueOf(cant)));
    ...
    impuestos = base.multiply(tasaIva != null ? tasaIva : BigDecimal.ZERO).setScale(2, RoundingMode.HALF_UP);


    total = base.add(impuestos).setScale(2, RoundingMode.HALF_UP);
    actualizandoEn= java.time.LocalDateTime.now();
